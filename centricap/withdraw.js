...
function Withdraw({user}){
    ...
    function withdraw(e){
        e.preventDefault()
        const url='/app/withdraw.php?key='+user+'&amount='+amount
        fetch(url)
        .then((res)=>res.json())
        .then((resp)=>{
            setMsg(resp)
    ...
    return (
        <div style={styles.main}>
            <form>
                ...
                <br/>
                <input type='text' placeholder='Amount to withdraw' style={styles.input} value={amount} onChange={stamount}/>
                <div style={styles.buttons}>
                    <button onMouseEnter={btnstate} onMouseLeave={btnleave} style={btn1} onClick={(e)=>withdraw(e)}>Withdraw</button>
                </div>
            </form>
        </div>
    )
